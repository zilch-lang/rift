  ...
  liftIO (findExecutable "git") >>= \ case
    ...
    Just gitExe -> do
      liftIO $ withLockFile (riftHome </> "package-set.lock") do
        (exit, out, err) <- procStrictWithErr (Text.pack gitExe) [ "-C", Text.pack pkgsHome, "tag", "-l", "-n", "1", "--color=never" ] empty
        ...
        let restoreToUnstable = do
              (exit, out, err) <- procStrictWithErr (Text.pack gitExe) [ "-C", Text.pack pkgsHome, "checkout", "unstable" ] empty
              unless (exit == ExitSuccess) do
